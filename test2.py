...
#    r = requests.post('http://127.0.0.1:4242/index.php', data=data, headers=headers)
    ...
    return parse_qs(r.content.decode('utf-8').strip())
...
print(r, "\n")
collection = json.loads(r['collection'][0])
...
def is_mate(data):
    ...
    if 'partner' not in data:
        return False
    p = data['partner'][0].split('~')
    if len(p) < 4:
        return False
    skin = p[1]
    color = int(p[2])
    spot = int(p[3])
    if skin not in collection:
        return True
    if spot < 4:
        return False # XXX
    collected = int(collection[skin].split('~')[1], 16)
    result = ((collected >> (color << 3)) >> spot) & 1
...
while True:
    ...
    if p:
        if 'partner' in p:
            if is_mate(p):
                actions.append('MATE|' + my_fid)
            else:
                actions.append('SPURN|' + my_fid)
    ...
    if level >= 8:
        if p and 'partner' in p and is_mate(p):
            pass
        else:
            done = True
    if done:
